...
import { getBoletinesAdmin, eliminarBoletin } from "../../api/boletines";

function AdminBoletines() {
  ...
  const cargar = async () => {
    ...
    setError("");
    try {
      setItems(await getBoletinesAdmin());
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-primary">Boletines</h1>
          <p className="mt-0.5 text-sm text-slate-500">{items.length} boletines</p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-500">
          No hay boletines agregados.{" "}
          <Link to="/admin/boletines/nuevo" className="font-semibold text-primary hover:underline">
            Agrega el primero
          </Link>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-soft">
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-5 py-3 text-left">Boletín</th>
                <th className="hidden px-5 py-3 text-left md:table-cell">Fecha</th>
                <th className="hidden px-5 py-3 text-left md:table-cell">Tipo</th>
                ...
                <tr key={b.id} className="transition hover:bg-slate-50">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="line-clamp-1 font-medium text-slate-800">{b.titulo}</span>
                    </div>
                  </td>
                  <td className="hidden px-5 py-4 text-slate-500 md:table-cell">
                    {new Date(b.fecha).toLocaleDateString("es-CL", { year: "numeric", month: "long" })}
                  </td>
                  <td className="hidden px-5 py-4 md:table-cell">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      b.isPdf ? "bg-secondary/15 text-secondary" : "bg-blue-100 text-blue-600"
                    }`}>
                      {b.isPdf ? "PDF" : "Enlace"}
                    </span>
                  </td>
                  <td className="hidden px-5 py-4 md:table-cell">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      b.activo ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"
                    }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center gap-3 text-amber-600">
              ...
              <h2 className="font-heading text-lg font-semibold">¿Eliminar boletín?</h2>
            </div>
