...
import { Plus, Pencil, Trash2, AlertTriangle, Image } from "lucide-react";
import { getGaleriaAdmin, eliminarFoto } from "../../api/galeria";
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-primary">Galería</h1>
          <p className="mt-0.5 text-sm text-slate-500">{items.length} fotos</p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-500">
          ...
          <Link to="/admin/galeria/nueva" className="font-semibold text-primary hover:underline">
            Agrega la primera
          </Link>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="hidden px-5 py-3 text-left md:table-cell">Caption</th>
                <th className="hidden px-5 py-3 text-left lg:table-cell">Orden</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((foto) => (
                <tr key={foto.id} className="transition hover:bg-slate-50">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="line-clamp-1 max-w-[200px] font-mono text-xs text-slate-500">
                        {foto.url}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="hidden px-5 py-4 lg:table-cell">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${foto.activo ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center gap-3 text-amber-600">
              ...
              <h2 className="font-heading text-lg font-semibold">¿Eliminar foto?</h2>
            </div>
