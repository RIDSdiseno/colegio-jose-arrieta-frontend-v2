...
const buttons = [
  {
    ...
    text: "text-primary",
    internal: true,
    onClick: () => trackPostulacionClick("boton_flotante"),
  },
  {
...
function FloatingButtons() {
  return (
    <div className="fixed right-0 top-1/2 z-40 -translate-y-1/2 flex flex-col gap-0.5">
      {buttons.map(({ label, href, icon: Icon, bg, text, internal, onClick }) => {
        ...
        if (internal) {
          return (
            <Link key={label} to={href} onClick={onClick} className={classes} aria-label={label}>
        ...
        return (
          <a key={label} href={href} target="_blank" rel="noreferrer" onClick={onClick} className={classes} aria-label={label}>
            <Icon className="h-4 w-4 shrink-0" aria-hidden="true" />
            <span className="max-w-0 overflow-hidden whitespace-nowrap text-xs font-bold transition-all duration-200 group-hover:max-w-xs">
              {label}
            </span>
